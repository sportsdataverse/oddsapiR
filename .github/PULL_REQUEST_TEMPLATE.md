...
- [ ] Verified against the live API with a valid `ODDS_API_KEY` (no key committed)
...
- [ ] `@return` tables use the 3-column `col_name | types | description` format
...
> Use Conventional Commits: `feat(toa):`, `fix(toa):`, `docs(pkgdown):`, `test(toa):`, etc.
